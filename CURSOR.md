...
- When this Cursor session is SoftOS orchestrator, apply
  `.agents/skills/softos-coding-execution-supervisor/SKILL.md`: delegate workers
  only with host `python3 ./flow agent run ...` and the coding-execution-runtime
  Priority table. Do not use Cursor Task/subagent panels as SoftOS workers.
